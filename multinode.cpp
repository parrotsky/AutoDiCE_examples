...
void model0_engine(const char* imagepath, ncnn::Mat& data_0, ncnn::Mat& conv1_2){
...
void model1_engine(int index, ncnn::Mat& conv1_2, ncnn::Mat& prob_1){
   MPI_Request requests[1];
   MPI_Status status[1];
...
static int multi_classify(const char* imagepath, std::vector<float>& cls_scores)
{
  ...
  if (irank == 0) {
      ...
      model0_engine(imagepath, data_0, conv1_2);
   }
  if (irank == 1) {
       ncnn::Mat conv1_2(54, 54, 96);
       ncnn::Mat prob_1;
 
      model1_engine(0, conv1_2, prob_1);
